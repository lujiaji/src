...
float fft(float* q, float* w, int n, int m, float sample_f) {
	...
	//MATH
		for(i=0; i<n; i+=2){
			if (i%(n/b)==0 && i!=0)
				k++;
			init_and_lookup(k, b, &cosine_value, &sine_value);
			real=mult_real(q[i+1], w[i+1], cosine_value, sine_value);
			imagine=mult_im(q[i+1], w[i+1], cosine_value, sine_value);
			new_[i]=q[i]+real;
